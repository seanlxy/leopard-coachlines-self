...
$total_rows =  fetch_value("SELECT COUNT(bp.`id`) 
	FROM `blog_post_has_category` bphc 
	LEFT JOIN `blog_post` bp
	ON (bphc.`post_id` = bp.`id`)
	LEFT JOIN `page_meta_data` pmd ON (bp.`page_meta_data_id` = pmd.`id`)
	LEFT JOIN `cms_users` cu ON (cu.`user_id` = pmd.`updated_by`)
	WHERE pmd.`status` = 'A'
	AND bphc.`category_id` = '{$category_data['id']}'
	ORDER BY bp.`date_posted` DESC");
...
if( !empty($posts_arr) )
{
	...
	$tags_arr['heading'] = '<h1 id="main-heading">Category Archives: '.$category_data['menu_label'].'</h1>';
