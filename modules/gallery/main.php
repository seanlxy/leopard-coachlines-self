...
{
    $gallery_photos = fetch_all("SELECT
                                    `full_path`,
                                    `thumb_path`,
                                ...
                                WHERE
                                    `photo_group_id` IN(
                                        ".implode(',', array_keys($gallery_ids))."
                                    )
                                ORDER BY
                                    `photo_group_id`,
                                    `rank`");
    ...
    if( !empty($gallery_photos) )
    {
        ...
        $gallery_list_view .= '<div class="container visible-xs mobile--heading text-center">
        ...
        foreach ($gallery_photos as $photo)
        {
            ...
            if($count == 0){
                ...
                $gallery_list_view .= '<figure class="col-xs-12 col-sm-4 first-img col-md-3 img ps-item" data-groups="[&#34;all&#34;,&#34;'.$photo['group_key'].'&#34;]" data-fpath="'.$photo['full_path'].'" data-size="'.$photo['size'].'" data-gp="'.$photo['group_key'].'" title="'.$photo['caption'].'">
                            <span'.$img.'></span>
                        </figure>';

            } else{
                $gallery_list_view .= '<figure class="col-xs-12 col-sm-4 other-img col-md-3 img ps-item" data-groups="[&#34;all&#34;,&#34;'.$photo['group_key'].'&#34;]" data-fpath="'.$photo['full_path'].'" data-size="'.$photo['size'].'" data-gp="'.$photo['group_key'].'" title="'.$photo['caption'].'">
                            <span'.$img.'></span>
                        </figure>';
            }
          $count++;
